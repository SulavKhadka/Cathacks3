...
def float_validator(raw_data):
    """Checks input for empty string and converts it to float 0.0
...
def weather_info(api_key):
    ...
    try: 
        ...
        return ["Please check to your internet connection"]
    try:
    ...
    visibility = float_validator(parsed_json['current_observation']['visibility_mi'])
    visible_condition = ("excellent" if visibility >= 10.0
                        else "good" if visibility >= 5.0
                        else "fair" if visibility >= 2.0
                        else "bad" if visibility > 0.5
                        else "very bad")
    precip = float_validator(parsed_json['current_observation']['precip_today_in'])
    if precip <= 0.0:
